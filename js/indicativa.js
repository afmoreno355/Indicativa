...
function llenarForm(numero, nombre, activo){
    ...
    if(numero>=activo && numero!==null){
        ...
        if(cuartoT>0){
                ...
                document.getElementsByClassName(nombre)[9].innerHTML=`<button type='button' id='button' name='${nombre}' onclick='enviarInfo(this.name, ${activo}, ${numero}, ${pt}, ${st}, ${tt}, ${ct})' title=''><i class='far fa-check-square botonModal'></i></button></pre>`;
        }
    }else{
...
function enviarInfo(nombre, activos , meta, pt, st, tt, ct ){
    idexistentesReCa('',`accionU=INDICATIVA&meta=${meta}&activos=${activos}&nombre=${nombre}&pt=${pt}&st=${st}&tt=${tt}&ct=${ct}`,'avisoIndicativa','View/CatalogoIndicativa/CatalogoIndicativaModificar.php')
}
...
function validarDatos(id, postcad, donde, accion){
    ...
    window.setTimeout( ()=> {           
        if(document.getElementById('avisoIndicativa')!==null){
            var names =['TECNICO', 'TECNICOINTEGRACION','AUXILIAR','OPERARIO','TECNOLOGO','ESPECIALIZACION TECNOLOGICA'];
