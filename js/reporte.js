...
}else if (document.getElementById('id').value==3 || document.getElementById('id').value==6) {
    for (var i = 0; i < cortar.length ; i++) {
        datos[i]=cortar[i].split('<|');
         parametros.push(datos[i][1]);  
...
function meses(valor, bucarPalabraClave){
    idexistentesGraficas('',`bucarPalabraClave=${bucarPalabraClave}&id=${valor}&centroGestion=`+document.getElementById('centroGestion').value ,'tableIntT','View/Reporte/ReporteTabla.php')
}